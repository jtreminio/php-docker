...
RUN apt-get update &&\
    apt-get -s dist-upgrade |grep "^Inst" |grep -i securi || true &&\
    apt-get autoclean &&\
...
ENV PHP.allow_url_fopen=1 \
    ...
    PHP.xdebug.client_discovery_header= \
    PHP.xdebug.client_host=host.docker.internal \
    PHP.xdebug.client_port=9003 \
    PHP.xdebug.collect_assignments=Off \
    PHP.xdebug.collect_return=Off \
    PHP.xdebug.connect_timeout_ms=200 \
    PHP.xdebug.discover_client_host=false \
